...
function scrollPositionToRGB(scrollTop) {
    ...
    const colorIndex = Math.floor(scrollTop / scrollRange * (colors.length - 1));
  
    // How far the user has scrolled within the current color section
    const intraSectionScroll = scrollTop % (scrollRange / (colors.length - 1));
  
    // The proportion of the way through the current color section
    const lerpAmount = intraSectionScroll / (scrollRange / (colors.length - 1));
